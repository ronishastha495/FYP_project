...
class VehicleBookingTestCase(TestCase):
    def setUp(self):
    ...
    def test_create_vehicle_booking(self):
        try:
            booking = VehicleBooking.objects.create(
                ...
                notes="Interested in purchasing this vehicle."
            )
            self.assertIsNotNone(booking)
            self.assertEqual(booking.user, self.user)
            self.assertEqual(booking.vehicle, self.vehicle)
            self.assertEqual(booking.status, "pending")
            print("Test Successful: Vehicle Booking Created")
        except Exception as e:
            print(f"Test Failed: Vehicle Booking Creation Error - {str(e)}")
            self.fail(f"Vehicle booking creation failed: {str(e)}")

    def test_vehicle_booking_str_method(self):
        try:
            booking = VehicleBooking.objects.create(
                booking_id=uuid.uuid4(),
                user=self.user,
            ...
            expected_str = f"Vehicle purchase booking for {self.user.username} on {booking.date} at {booking.time}"
            self.assertEqual(str(booking), expected_str)
            print("Test Successful: Vehicle Booking String Method Verified")
        except Exception as e:
            print(f"Test Failed: Vehicle Booking String Method Error - {str(e)}")
            self.fail(f"Vehicle booking string method failed: {str(e)}")
