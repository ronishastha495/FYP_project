...
const ChatBox = ({ selectedUser }) => {
    ...
    return (
      <div className="flex items-center justify-center h-full text-indigo-400 text-lg font-medium">
        <div className="text-center p-8 bg-white/50 backdrop-blur-sm rounded-2xl shadow-lg">
          <svg className="w-16 h-16 mx-auto mb-4 text-indigo-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-lg overflow-hidden border border-indigo-100">
      ...
      <div
        className="flex-1 h-56 overflow-y-auto p-5 space-y-4 bg-indigo-50/30 custom-scrollbar"
      >
        {messages.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <p className="text-indigo-300 text-center">No messages yet. Say hello!</p>
          </div>
        ) : (
          messages.map((msg, i) => (
            <div
              ...
              >
                ...
                <span
                  className={`text-xs block mt-1 ${
                    msg.sender === currentUserId ? 'text-indigo-100' : 'text-gray-400'
                  }`}
                >
