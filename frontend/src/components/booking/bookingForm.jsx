...
const BookingForm = ({
  ...
  const { user } = useAuth();
  console.log("User in BookingForm:", user);
  const [step, setStep] = useState(1);
  const [bookingType, setBookingType] = useState(
    initialBookingType || (service ? "servicing" : vehicle ? "purchase" : "")
  );
  ...
  const [formData, setFormData] = useState({
    ...
    time: "",
    notes: "",
    purchase_details: vehicle
      ? `Interested in ${vehicle.make} ${vehicle.model} (${vehicle.year})`
      : "",
    trade_in_vehicle_id: "",
  });
  ...
  const vehicles =
    bookingType === "servicing" ? customerVehicles : dealershipVehicles;
  ...
  const handleBookingTypeSelect = (type) => {
    ...
    setFormData({
      ...
      booking_type: type,
      vehicle_context:
        type === "servicing" ? "customer_owned" : "dealership_vehicle",
      vehicle_id: "",
      service_id: type === "servicing" ? service?.id || "" : "",
      purchase_details:
        type === "purchase" && vehicle
          ? `Interested in ${vehicle.make} ${vehicle.model} (${vehicle.year})`
          : "",
    });
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const bookingData = {
        ...
        vehicle: formData.vehicle_id, // Direct ID
        vehicle_context: formData.vehicle_context, // Separate field
        date: formData.date,
        time: formData.time,
        notes: formData.notes,
        ...(formData.booking_type === "servicing"
          ? { primary_service: formData.service_id }
          : {
              purchase_details: formData.purchase_details,
              ...(formData.trade_in_vehicle_id && {
                trade_in_vehicle: formData.trade_in_vehicle_id,
                trade_in_vehicle_context: "customer_owned",
              }),
            }),
      };
      ...
      toast.error(error.error || error.message || "Failed to create booking");
    } finally {
  ...
  const renderStepContent = () => {
    // Get services from context at the component level
    // const { services, loading: servicesLoading } = useContext(BookingContext);
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <p className="text-sm text-gray-600">
                  Book a service for your vehicle
                </p>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <p className="text-sm text-gray-600">
                  Book a test drive or purchase consultation
                </p>
              </button>
            </div>
          </div>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setFormData({
                            ...formData,
                            trade_in_vehicle_id:
                              formData.trade_in_vehicle_id === v.id ? "" : v.id,
                          })
                        }
                        className={`p-3 border rounded-lg text-left ${
                          formData.trade_in_vehicle_id === v.id
                            ? "border-blue-500 bg-blue-50"
                            : "border-gray-300"
                        }`}
                      >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <p>
                ...
                {vehicles.find((v) => v.id === formData.vehicle_id)?.make}{" "}
                {vehicles.find((v) => v.id === formData.vehicle_id)?.model}
              </p>
              {bookingType === "servicing" && (
                <p>
                  <strong>Service:</strong>{" "}
                  {services?.find((s) => s.id === formData.service_id)?.name || "No service selected"}
                </p>
              )}
              {bookingType === "purchase" && (
                <div>
                  ...
                  {formData.trade_in_vehicle_id && (
                    <p>
                      <strong>Trade-in Vehicle:</strong>{" "}
                      {
                        customerVehicles?.find(
                          (v) => v.id === formData.trade_in_vehicle_id
                        )?.make
                      }{" "}
                      {
                        customerVehicles?.find(
