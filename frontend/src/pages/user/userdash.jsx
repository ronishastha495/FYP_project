  ...
  useEffect(() => {
    const fetchUserBookings = async () => {
      ...
      try {
        ...
        // Transform booking data to match appointments format
        const formattedBookings = bookingsData.map(booking => ({
          id: booking.id,
          vehicle: booking.vehicle_details ? 
            `${booking.vehicle_details.make || ''} ${booking.vehicle_details.model || ''}` : 
            'Vehicle',
          service: booking.booking_type === 'servicing' ? 
            (booking.primary_service?.name || 'Service') : 
            'Vehicle Purchase',
          date: booking.date,
          time: booking.time,
          price: booking.final_cost || 0,
